...
保存は atomic に扱う。どれか 1 entry でも bookmark 生成、encode、write に失敗した場合は partial save を行わず、直前の persisted state を保持する。
...
`playlistState` キーが存在するのに decode 失敗、または schema 不整合で復元不能な場合は、その key を破損 state とみなして削除し、1 回だけ legacy 単一 bookmark への fallback を試す。fallback にも失敗した場合は空 playlist に戻す。

### Legacy Migration

`playlistState` がなければ、既存の単一 bookmark を 1-item playlist に変換する。その後、新形式で即保存する。移行成功後は legacy の `videoBookmark` を削除し、以後は `playlistState` だけを source of truth とする。

playlist clear 時も `playlistState` と `videoBookmark` の両方を削除する。

`playlistState` が空 state として正しく復元できた場合は、それを正規状態として扱い、legacy 単一 bookmark へは fallback しない。
...
Branch 2 では、現在の `AVPlayerLooper` ベースの無限ループを current item 再生の基本 primitive から外し、`1 セグメントを単発再生して完了通知を受ける` モデルへ置き換える。ループ継続は looper ではなく、playlist advance の wrap によって実現する。
